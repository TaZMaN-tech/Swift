...
protocol Car {
    ...
    var startEngine: EngineStatus { get set }
    var windowsStatus: WindowsStatus { get set }
    var maxVolume: Double { get set }
    var filledVolume: Double { get set }
...
extension Car {
    mutating func startEngineOn () {
        startEngine = EngineStatus.on
    }
}

extension Car {
    mutating func windowStatusClose () {
...
print(car1)
var car2 = SportCar(mark: "Honda", year: 2015, startEngine: .off, windowsStatus: .open, maxVolume: 200, filledVolume: 37, hatchState: .open)
car2.changeEngine(state: .off)
car2.changeTrunk(load: .unload(volume: 3))
print(car2)
var car3 = TrunkCar(mark: "MAZ", year: 1977, startEngine: .on, windowsStatus: .close, maxVolume: 800, filledVolume: 578, statusTrailerAttached: .no)
car3.changeEngine(state: .on)
car3.changeTrunk(load: .unload(volume: 567))
print(car3)
var car4 = TrunkCar(mark: "Kamaz", year: 1988, startEngine: .off, windowsStatus: .open, maxVolume: 600, filledVolume: 100, statusTrailerAttached: .yes)
car4.changeEngine(state: .off)
car4.changeTrunk(load: .load(volume: 400))
print(car4.printCar())
